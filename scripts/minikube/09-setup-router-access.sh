...
if [ ! -d "scripts/minikube" ] || [ ! -d "subgraphs" ] || [ ! -d "deploy" ]; then
    ...
    echo "Please run: ./scripts/minikube/09-setup-router-access.sh"
    exit 1
fi

echo "=== Step 09: Setting Up Router Access ==="
...
if grep -q "^export ROUTER_URL=" "$ENV_FILE"; then
    ...
    echo "" >> "$ENV_FILE"
    echo "# Router URL (generated by 09-setup-router-access.sh)" >> "$ENV_FILE"
...
echo ""
echo "Next: Run ./scripts/minikube/10-deploy-client.sh to deploy the client application"
